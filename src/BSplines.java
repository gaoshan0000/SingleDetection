public class BSplines {
    ...
    public static double [] bSplineBlender(int k, double s, double ... t) {
        return normalize(N(k, VectorMath.add(VectorMath.div(t, s), (double)k / 2.0)));   // scale and align to center, then evaluate, and finally normalize sum to 1
    }
    ...
    private static double[] normalize(double[] arr) {
        return VectorMath.div(arr, VectorMath.sum(arr));
    }
