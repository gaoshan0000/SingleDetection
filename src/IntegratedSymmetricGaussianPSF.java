        ...
        return new MultivariateMatrixFunction() {
            ...
            public double[][] value(double[] point) throws IllegalArgumentException {
                ...
                for(int i = 0; i < errDiffXs.length; i++) {
                    ...
                    double expplus = exp(-MathProxy.sqr(dxPlusHalf) / (2 * sigmaSquared)) * sqrt(2 / PI);
                    double expminus = exp(-MathProxy.sqr(dxMinusHalf) / (2 * sigmaSquared)) * sqrt(2 / PI);
                ...
                for(int i = 0; i < edge; i++) {
                    ...
                    double expplus = exp(-MathProxy.sqr(dyPlusHalf) / (2 * sigmaSquared)) * sqrt(2 / PI);
                    double expminus = exp(-MathProxy.sqr(dyMinusHalf) / (2 * sigmaSquared)) * sqrt(2 / PI);
...
//                IJ.log("analytic jacobian: " + Arrays.deepToString(retVal));
    ...
    @Override
    public Molecule newInstanceFromParams(double[] params, boolean afterFitting) {
